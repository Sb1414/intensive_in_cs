...
try
{
    ...
    for (int paymentNumber = 1; paymentNumber <= numberOfMonths; paymentNumber++)
    {
        ...
        Console.WriteLine($"{paymentNumber,-9}\t{paymentDate.ToString("MM/dd/yyyy"),-12}\t{monthlyPayment.ToString("N2"),13}\t" +
        $"{principalPayment.ToString("N2"),12}\t{interestPayment.ToString("N2"),15}\t{roundedTotalDebtBalance.ToString("N2"),15}");
